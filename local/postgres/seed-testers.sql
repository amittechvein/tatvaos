...
--  a suspended person, a disabled mailbox, long folder contents. A two-row
...
-- ----------------------------------------------------------------------------
INSERT INTO core.tenants (id, name, type, status) VALUES
    ('33333333-3333-3333-3333-333333333333', 'City Clinic', 'hospital', 'active')
ON CONFLICT (id) DO NOTHING;

INSERT INTO core.domains (id, tenant_id, fqdn, type, is_active, ownership_verified_at) VALUES
    ('c3333333-3333-3333-3333-333333333333',
     '33333333-3333-3333-3333-333333333333',
     'cityclinic.local', 'primary', true, now())
ON CONFLICT (fqdn) DO NOTHING;

INSERT INTO core.storage_pools (tenant_id, storage_model, per_user_quota_bytes) VALUES
    ('33333333-3333-3333-3333-333333333333', 'per_user', 1073741824)
ON CONFLICT (tenant_id) DO NOTHING;

-- Dr Sharma is a person. reception@ is not — it is a shared mailbox with no
-- one behind it, which is the case that justifies keeping the two apart.
INSERT INTO core.users (id, tenant_id, domain_id, email, display_name, role, status) VALUES
    ('d3333333-3333-3333-3333-333333333333',
     '33333333-3333-3333-3333-333333333333',
     'c3333333-3333-3333-3333-333333333333',
     'dr.sharma@cityclinic.local', 'Dr Sharma', 'employee', 'active')
ON CONFLICT (id) DO NOTHING;
...
-- ----------------------------------------------------------------------------
INSERT INTO core.tenants (id, name, type, status) VALUES
    ('44444444-4444-4444-4444-444444444444', 'Rival Corp', 'business', 'active')
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO core.storage_pools (tenant_id, storage_model, per_user_quota_bytes) VALUES
    ('44444444-4444-4444-4444-444444444444', 'per_user', 15728640000)
ON CONFLICT (tenant_id) DO NOTHING;
...
INSERT INTO core.users (id, tenant_id, domain_id, email, display_name, role, status) VALUES
    ('d4444444-4444-4444-4444-444444444445',
     '44444444-4444-4444-4444-444444444444',
     'd4444444-4444-4444-4444-444444444444',
     'ceo@rivalcorp.local', 'Rival CEO', 'owner', 'active')
ON CONFLICT (id) DO NOTHING;

INSERT INTO mail.mailboxes (tenant_id, domain_id, user_id, address, local_part,
                            type, imap_password_hash) VALUES
...
-- A departed employee. THE case that Core exists for: the person is suspended,
-- so they cannot sign in to Mail or anything else — but the mailbox is kept
-- for the retention window and mail to it is REJECTED, not silently accepted.
INSERT INTO core.users (id, tenant_id, domain_id, email, display_name, role, status) VALUES
    ('d1111111-1111-1111-1111-111111111199',
     '11111111-1111-1111-1111-111111111111',
     'a1111111-1111-1111-1111-111111111111',
     'former.employee@techvein.local', 'Former Employee', 'employee', 'suspended')
ON CONFLICT (id) DO NOTHING;

INSERT INTO mail.mailboxes (tenant_id, domain_id, user_id, address, local_part,
                            type, imap_password_hash, is_active) VALUES
...
CROSS  JOIN LATERAL (SELECT id FROM mail.mailboxes WHERE address='amit@techvein.local') m
ON CONFLICT (address) DO NOTHING;
...
-- ----------------------------------------------------------------------------
DO $$
DECLARE t int; d int; u int; mb int; a int; ms int;
BEGIN
    SELECT count(*) INTO t  FROM core.tenants;
    SELECT count(*) INTO d  FROM core.domains;
    SELECT count(*) INTO u  FROM core.users;
    SELECT count(*) INTO mb FROM mail.mailboxes;
    SELECT count(*) INTO a  FROM mail.aliases;
    SELECT count(*) INTO ms FROM mail.messages;
    RAISE NOTICE '';
    RAISE NOTICE '  Tester seed applied';
    RAISE NOTICE '    tenants % | domains % | users % | mailboxes % | aliases % | messages %',
                 t, d, u, mb, a, ms;

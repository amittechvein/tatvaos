...
**What logs today, checked 13 Sept.** `infra/docker/Caddyfile` and every
`conf.d/*.caddy` fragment have no `log` directive, and the running Caddy
container printed zero request lines in ten minutes: Caddy access logging is
off. Next's standalone server does not log requests. So a query-string code
would not land in a server log *today* — the ruling is about the day
somebody turns logging on, and about browser history, which is a log we do
not control.
...
`{ "path": "/mail/inbox" }` (the product path to land on; must match an
allowlist of our own first path segments — `mail`, `space`, `calendar`,
`family`, `admin` — anything else is refused, so the handoff cannot become
an open redirect). Response:
...
path`. Zero rows means invalid, used, or expired — the same 401 for all
three, so a probe cannot tell them apart. Then the same checks login makes
(`users.status = 'active'`, `tenants.status IN ('active','trial')`), then the
same cookies login sets, then `{ "redirect": path }`; the page navigates.
Rate limit: five redeems a minute per IP, through the existing limiter.
...
A second client (desktop, a partner) needs the same handoff. Then the
allowlist and the mint become a general "session exchange" and this record
is superseded.

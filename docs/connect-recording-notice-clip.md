...
| loudness | about −16 LUFS | **−24.0 LUFS** integrated (ebur128) — deliberately quieter, see below |
| peak | no higher than −3 dBFS | −11.4 dBFS |
...
The command, so the next version can be made the same way:
    ...
    ffmpeg -ss 0.70 -to 2.45 -i <voice-note>.ogg -map_metadata -1 \
      -af "aresample=44100,volume=6.5dB,alimiter=limit=0.70:attack=1:release=40:level=disabled,volume=0.4,afade=t=in:d=0.02,areverse,afade=t=in:d=0.03,areverse" \
      -ac 1 -ar 44100 -c:a libmp3lame -b:a 96k -id3v2_version 0 -write_xing 0 connect-recording-notice.mp3

**Heard in a live meeting, 17 Sept 2026, and turned down.** The first cut met
the −16 LUFS target exactly and Amit reported it "very high volume", asking
for it 60% quieter. `volume=0.4` after the limiter is 40% of the amplitude,
−7.9 dB: measured −24.0 LUFS, −11.4 dBFS peak. The −16 LUFS target in the brief
below was a number chosen without hearing it over a meeting; this is the level
chosen by listening, and it wins. The file is otherwise identical (1.78 s,
21,316 bytes).

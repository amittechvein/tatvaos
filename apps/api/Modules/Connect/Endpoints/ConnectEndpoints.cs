...
public static class ConnectEndpoints
{
    ...
    {
        if (tenant.UserId is not Guid uid) return Results.Unauthorized();
        return await CreateMeetingForAsync(uid, req, db, tenant, hasher, roomKeys, audit, logs, ct);
    }

    /// <summary>
    /// Creating a meeting ON BEHALF OF a named host, which is the only thing
    /// the signed-in path above does differently: it reads the host from the
    /// session, and there isn't one when a school's ERP calls the
    /// organisation API with a key (Amit, 18 September 2026).
    ///
    /// EXTRACTED RATHER THAN COPIED, for the reason UserEndpoints.CreatePersonAsync
    /// was: every rule below — the mode lock, the private-meeting refusal
    /// when no room key is configured, the password length, the waiting-room
    /// values, the auto-record constraint the database also enforces — must
    /// apply identically whoever is asking. A second creation path would
    /// start identical and would not stay that way, and the rule that drifted
    /// would be found by whichever customer it let down.
    ///
    /// The ONE thing the caller must still do is decide who may act as this
    /// host. This method does not: it trusts hostUserId. The organisation API
    /// establishes it from a verified key plus an address in that tenant
    /// before calling.
    /// </summary>
    /// <summary>
    /// Somewhere for a caller that is not an HTTP response to pick the meeting
    /// up. The organisation API answers in its own shape — a school's ERP
    /// should not have to parse Connect's internal one — and needs the row
    /// rather than the IResult wrapping it.
    ///
    /// An out-parameter in all but name, and deliberately NOT a refactor of
    /// this method into one that returns (error, meeting): that would rewrite
    /// a dozen early returns, each of which is a rule somebody got wrong once,
    /// for the benefit of one caller. One optional field and one assignment
    /// changes nothing about how the rules run.
    /// </summary>
    internal sealed class Sink { public ConnectMeeting? Meeting; }

    internal static async Task<IResult> CreateMeetingForAsync(
        Guid uid, CreateMeetingRequest req, AppDbContext db, TenantContext tenant,
        IPasswordHasher hasher, ConnectRoomKey roomKeys, AuditWriter audit,
        ILoggerFactory logs, CancellationToken ct, Sink? sink = null)
    {
        ...
        //  After the save, not before: the meeting's id is generated by the
        //  database, and the calendar row is keyed on it.
        //
        //  A SECOND SAVE, AND A SWALLOWED FAILURE. A meeting with no calendar
        //  row is a meeting you can still join; a meeting that failed to be
        //  created because a calendar write threw is not. The calendar is the
        //  lesser of the two, so it is never allowed to take the meeting down
        //  with it. The warning is the thing that says so out loud, because a
        //  silently missing calendar entry is precisely the class of bug this
        //  codebase keeps paying for.
        await MirrorToCalendarAsync(db, meeting, uid, logs, ct);

        if (sink is not null) sink.Meeting = meeting;
        return Results.Created($"/api/connect/meetings/{meeting.Id}", Shape(meeting, "host"));
    }

    /// <summary>
    /// Keep the host's calendar row in step with the meeting, and never fail
    /// the caller over it. See ConnectCalendarMirror for why a row exists at
    /// all rather than Calendar reading connect.meetings.
    /// </summary>
    private static async Task MirrorToCalendarAsync(
        AppDbContext db, ConnectMeeting meeting, Guid hostUserId, ILoggerFactory logs, CancellationToken ct)
    {
        try
        {
            await ConnectCalendarMirror.UpsertAsync(db, meeting, hostUserId, JoinUrlOf(meeting), ct);
            await db.SaveChangesAsync(ct);
        }
        catch (Exception ex)
        {
            // ── SWALLOWING IS NOT ENOUGH; THE CONTEXT HAS TO BE CLEANED ────
            //
            //  A SaveChangesAsync that threw leaves its entities tracked and
            //  still pending. Every later SaveChangesAsync on this request —
            //  the audit rows a caller writes after this, the re-permissioning
            //  on an update — would try them again and throw the same
            //  exception, which would arrive as a 500 from a line that has
            //  nothing to do with calendars. The calendar would have failed
            //  quietly and taken the meeting's own update down with it, which
            //  is exactly what this try/catch exists to prevent.
            //
            //  So the mirror's own pending work is detached. Only these two
            //  types: anything else pending belongs to the caller and is its
            //  business, not ours.
            foreach (var e in db.ChangeTracker.Entries<CalendarEvent>().ToList())
                e.State = EntityState.Detached;
            foreach (var e in db.ChangeTracker.Entries<CalendarCalendar>().ToList())
                e.State = EntityState.Detached;

            logs.CreateLogger(typeof(ConnectEndpoints)).LogWarning(
                ex, "Meeting {MeetingId} could not be put on a calendar; the meeting itself is unaffected.",
                meeting.Id);
        }
    }
    ...
    private static async Task<IResult> UpdateMeetingAsync(
        ...
        TatvaOS.Api.Modules.Family.ContactAutoSave autoSave, ILoggerFactory logs,
        CancellationToken ct)
    {
        ...
        // The calendar carries the time and the title, so a reschedule that
        // did not reach it would leave the host's own calendar pointing at
        // the old slot — the exact failure the emailed update exists to
        // prevent for everyone else. Host id from the row, not from `uid`: a
        // co-host may be the one editing, and the entry belongs to the host.
        await MirrorToCalendarAsync(db, meeting, meeting.CreatedByUserId ?? uid, logs, ct);
    ...
    private static async Task<IResult> CancelMeetingAsync(
        ...
        ILogger<LiveKitRoomClient> log, ILoggerFactory logs, CancellationToken ct)
    {
        if (tenant.UserId is not Guid uid) return Results.Unauthorized();
        var meeting = await FindAsync(db, id, ct);
        if (meeting is null) return NotFound();
        ...
        // Off the host's own calendar as well as the calendars it was emailed
        // to. Same reasoning as the CANCEL below: a cancelled class must not
        // keep sitting on anybody's Monday, and the host is somebody.

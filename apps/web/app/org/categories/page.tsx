          ...
          <div key={c.id} className="rounded-xl border border-gray-200 bg-white p-5">
            ...
            <dl className="space-y-1.5 text-sm">
              <div className="flex justify-between">
                <dt className="text-gray-500">Default quota</dt>
              ...
              <div className="flex justify-between">
                ...
                <dd className="capitalize text-gray-900">{c.defaultRole.replace(/_/g, ' ')}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-gray-500">Send outside org</dt>
                <dd className={c.canSendExternal ? 'text-gray-900' : 'font-medium text-amber-700'}>
                  {c.canSendExternal ? 'Allowed' : 'Blocked'}
                </dd>
              </div>
              {c.autoGroups && c.autoGroups.length > 0 && (
                <div className="flex justify-between gap-4">
                  <dt className="shrink-0 text-gray-500">Auto groups</dt>
                  <dd className="truncate text-right text-gray-900">{c.autoGroups.join(', ')}</dd>
                </div>
              )}
            </dl>
